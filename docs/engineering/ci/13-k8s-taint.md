...
![](https://s.poetries.top/images/202202231501344.png)
...
> 其中，Node\_Name 为要添加污点的 node 名称；key 和 value 为一组键值对，代表一组标示标签；NoSchedule 则为不被调度的意思，和它同级别的还有其他的值：PreferNoSchedule 和 NoExecute （后面我们会写到）

我们给 Node3 添加完一个污点后，提示报 node/node2 tainted 代表添加成功：

![](https://s.poetries.top/images/202202231501241.png)
...
![](https://s.poetries.top/images/202202231501519.png)

> 这时候我们看到， Pod 被调度到了 Node1 上面去。因为 Node2 添加了污点，不会被调度到 Node2 上面去。此时污点生效。

## [#](#给-pod-设置容忍度) 给 Pod 设置容忍度
...
![](https://s.poetries.top/images/202202231502843.png)
...
![](https://s.poetries.top/images/202202231502096.png)

> 可以看到，在容忍度的作用下， Pod 重新被调度到了 Node2 节点上。

## [#](#修改-删除-node-的污点) 修改/删除 Node 的污点
