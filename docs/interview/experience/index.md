...
`getBoundingClientRect()` 可以单独拿出来记：

*   它返回元素相对于**视口**的位置和尺寸。
*   它常用于判断元素是否进入可视区、做吸顶定位、碰撞检测和懒加载。
*   它拿到的是实时布局结果，所以读取它时通常会触发布局计算，属于比较“重”的操作。

和几个容易混淆的属性对比一下：

| 属性 | 看什么 | 基准 | 常见用途 |
| --- | --- | --- | --- |
| `getBoundingClientRect()` | 元素当前在屏幕上的位置和尺寸 | 视口 | 判断可视区、定位、碰撞检测 |
| `offsetTop / offsetLeft` | 元素相对 `offsetParent` 的偏移 | 定位父级 | 布局计算、老代码兼容 |
| `clientTop / clientLeft` | 元素自己的边框宽度 | 元素自身 | 参与坐标换算 |
| `scrollTop / scrollLeft` | 内容已经滚动的距离 | 滚动容器自身 | 滚动监听、锚点定位 |

如果想拿到“相对于整个文档”的位置，一般要把视口坐标加上滚动距离：
