...
# [#](#nodejs进阶-crypto模块之理论篇) Nodejs进阶：crypto模块之理论篇
...
## [#](#六、分组加密模式、填充、初始化向量) 六、分组加密模式、填充、初始化向量
...
图片来源：[这里 (opens new window)](https://en.wikipedia.org/wiki/Block_cipher_mode_of_operation#Electronic_Codebook_%28ECB%29)，更多关于分组加密模式的介绍可以参考 [wiki (opens new window)](https://en.wikipedia.org/wiki/Block_cipher_mode_of_operation#Common_modes)。

> 后面假设每个块的长度为128位

### [#](#_2、初始化向量-iv) 2、初始化向量：IV
...
如图所示，图片来源 [这里 (opens new window)](https://en.wikipedia.org/wiki/Block_cipher_mode_of_operation#Cipher_Block_Chaining_%28CBC%29)

### [#](#_3、填充-padding) 3、填充：padding
...
[RSA的公钥和私钥到底哪个才是用来加密和哪个用来解密？ - 刘巍然-学酥的回答 - 知乎 (opens new window)](https://www.zhihu.com/question/25912483/answer/31653639)
