...
### [#](#方案一-多个node实例-多个端口) 方案一：多个node实例+多个端口
...
### [#](#方案二-主进程向子进程转发请求) 方案二：主进程向子进程转发请求
...
## [#](#问题1-master、worker如何通信) 问题1：master、worker如何通信

这个问题比较简单。master进程通过 cluster.fork() 来创建 worker进程。cluster.fork() 内部 是通过 child\_process.fork() 来创建子进程。
...
### [#](#问题3-如何将请求分发到多个worker) 问题3：如何将请求分发到多个worker

每当worker进程创建server实例来监听请求，都会通过IPC通道，在master上进行注册。当客户端请求到达，master会负责将请求转发给对应的worker。

具体转发给哪个worker？这是由转发策略决定的。可以通过环境变量NODE\_CLUSTER\_SCHED\_POLICY设置，也可以在cluster.setupMaster(options)时传入。

默认的转发策略是轮询（SCHED\_RR）。

当有客户请求到达，master会轮询一遍worker列表，找到第一个空闲的worker，然后将该请求转发给该worker。

## [#](#master、worker内部通信小技巧) master、worker内部通信小技巧
