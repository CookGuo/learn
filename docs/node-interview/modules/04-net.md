...
## [#](#简单的-server-client-例子) 简单的 server+client 例子
...
### [#](#server-ref-server-unref) server.ref()/server.unref()

了解node事件循环的同学对这两个API应该不陌生，主要用于将server 加入事件循环/从事件循环里面剔除，影响就在于会不会影响进程的退出。

对出学习net的同学来说，并不需要特别关注，感兴趣的自己做下实验就好。

### [#](#事件-listening-connection-close-error) 事件 listening/connection/close/error
...
官方文档： https://nodejs.org/api/net.html#net\_socket\_destroy\_exception
