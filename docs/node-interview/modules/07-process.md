...
## [#](#环境变量-process-env) 环境变量：process.env
...
## [#](#异步-process-nexttick-fn) 异步：process.nextTick(fn)
...
## [#](#获取命令行参数-process-argv) 获取命令行参数：process.argv
...
## [#](#获取node-specific参数-process-execargv) 获取node specific参数：process.execArgv
...
## [#](#当前工作路径-process-cwd-vs-process-chdir-directory) 当前工作路径：process.cwd() vs process.chdir(directory)
...
## [#](#标准输入-标准输出-标准错误输出-process-stdin、process-stdout) 标准输入/标准输出/标准错误输出：process.stdin、process.stdout
...
3.  process.release：返回当前node发行版本的相关信息，大部分时候不会用到。具体字段含义可以看[这里 (opens new window)](https://nodejs.org/api/process.html#process_process_release)。
...
## [#](#警告信息-process-emitwarning-warning) 警告信息:process.emitWarning(warning);
...
## [#](#向进程发送信号-process-kill-pid-signal) 向进程发送信号：process.kill(pid, signal)
...
## [#](#终止进程-process-exit-exitcode-、process-exitcode) 终止进程：process.exit(\[exitCode\])、process.exitCode
...
[nodejs 异步之 Timer &Tick; 篇 (opens new window)](https://cnodejs.org/topic/4f16442ccae1f4aa2700109b)
