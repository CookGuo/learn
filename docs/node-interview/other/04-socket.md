...
### [#](#一、websocket-解决了什么问题) 一、WebSocket 解决了什么问题
...
![](https://s.poetries.top/gitee/2019/10/389.png)

### [#](#二、websocket的class类) 二、webSocket的class类
...
### [#](#三、websocket不稳定) 三、WebSocket不稳定

> `WebSocket`并不稳定，在使用一段时间后，可能会断开连接，貌似至今没有一个为何会断开连接的公论，所以我们需要让`WebSocket`保持连接状态，这里推荐两种方法

#### [#](#_3-1-websocket设置变量-判断是否手动关闭连接) 3.1 WebSocket设置变量，判断是否手动关闭连接
...
### [#](#_3-2-websocket心跳机制) 3.2 WebSocket心跳机制
...
### [#](#四、关于websocket) 四、关于WebSocket

#### [#](#_4-1-websocket的当前状态-websocket-readystate) 4.1 WebSocket的当前状态:`WebSocket.readyState`
...
#### [#](#_4-2-websocket还可以发送-接收-二进制数据) 4.2 `WebSocket`还可以发送/接收 二进制数据
...
### [#](#一、原生node与socket-io通信) 一、原生Node与socket.io通信
...
![](https://s.poetries.top/gitee/2019/10/390.png) ![](https://s.poetries.top/gitee/2019/10/391.png)

#### [#](#_1-3-服务器端通过emit广播-通过on接收广播) 1.3 服务器端通过emit广播，通过on接收广播
...
![](https://s.poetries.top/gitee/2019/10/392.png)
...
![](https://s.poetries.top/gitee/2019/10/393.png) ![](https://s.poetries.top/gitee/2019/10/394.png)

*   `io.emit()`可以实现聊天室消息群发
*   `socket.emit()`可以实现聊天机器人，一对一发送

#### [#](#_1-4-客户端端通过emit广播-通过on接收广播) 1.4 客户端端通过emit广播，通过on接收广播
...
### [#](#二、聊天室、智能机器人实现原理) 二、聊天室、智能机器人实现原理

#### [#](#_2-1-express简单例子) 2.1 express简单例子
...
#### [#](#_2-2-express实现智能机器人) 2.2 express实现智能机器人
...
#### [#](#_2-3-express结合socket-io及数据库实现智能机器人) 2.3 express结合`socket.io`及数据库实现智能机器人
...
### [#](#三、koa中socket-io的使用) 三、Koa中Socket.io的使用
...
> [完整代码 (opens new window)](https://github.com/poetries/socket.io-demo/tree/master/koa-socket.io)
