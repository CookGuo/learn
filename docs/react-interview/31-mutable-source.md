---
title: React 18特性篇 useMutableSource外部数据源|React进阶专题 | 前端进阶学习
source: https://interview.poetries.top/fe-improve-docs/react/react-improve/docs/31-V18特性篇%20useMutableSource外部数据源.html
crawled: 2026-04-09
---

# React 18特性篇 useMutableSource外部数据源|React进阶专题 | 前端进阶学习

> 原文: [https://interview.poetries.top/fe-improve-docs/react/react-improve/docs/31-V18特性篇%20useMutableSource外部数据源.html](https://interview.poetries.top/fe-improve-docs/react/react-improve/docs/31-V18特性篇%20useMutableSource外部数据源.html)

---

## [#](#一-前言) 一 前言

`useMutableSource` 最早的 [RFC (opens new window)](https://link.juejin.cn/?target=https%3A%2F%2Fgithub.com%2Freactjs%2Frfcs%2Fblob%2Fmain%2Ftext%2F0147-use-mutable-source.md) 提案在 2020年 2 月份就开始了。在 React 18 中它将作为新特性出现。用一段提案中的描述来概括 `useMutableSource`。
...
![1.jpg](https://s.poetries.top/md/0734aca032e046d58d9c263b6d3d5cb3~tplv-k3u1fbpfcp-zoom-in-crop-mark_1304_0_0_0.png?)

![2.jpg](https://s.poetries.top/md/dd8b70d275ad4f8aa84f18f4029960ea~tplv-k3u1fbpfcp-zoom-in-crop-mark_1304_0_0_0.png?)
...
useMutableSource 可以使用非传统的数据源。它的功能和 Context API 还有 [useSubscription (opens new window)](https://link.juejin.cn/?target=https%3A%2F%2Fwww.npmjs.com%2Fpackage%2Fuse-subscription) 类似。（没有使用过 useSubscription 的同学，可以了解一下 ）。
...
**关于 MutableSource 版本号** `useMutableSource` 会追踪 MutableSource 的版本号，然后读取数据，所以如果两者不一致，可能会造成读取异常的情况。useMutableSource 会检查版本号：
...
*   在最新的 react-redux 源码中，已经使用新的 api，订阅外部数据源，不过不是 `useMutableSource` 而是 `useSyncExternalStore`，具体因为 `useMutableSource` 没有提供内置的 selectorAPI，需要每一次当选择器变化时候重新订阅 store，如果没有 useCallback 等 api 记忆化处理，那么将重新订阅。具体内容请参考 [useMutableSource → useSyncExternalStore (opens new window)](https://link.juejin.cn/?target=https%3A%2F%2Fgithub.com%2Freactwg%2Freact-18%2Fdiscussions%2F86)。
...
![3.jpg](https://s.poetries.top/md/a000b3baac184fe9903700ad51e03be7~tplv-k3u1fbpfcp-zoom-in-crop-mark_1304_0_0_0.png?)
...
第一部分用 `combineReducers` 和 `createStore` 创建 redux Store 的过程。 重点是第二部分：
...
![4.gif](https://s.poetries.top/md/81ad31834d0941859cf767d22d685c6b~tplv-k3u1fbpfcp-zoom-in-crop-mark_1304_0_0_0.png?)

## [#](#四-原理分析) 四 原理分析

useMutableSource 已经在 React v18 的规划之中了，那么它的实现原理以及细节，在 V18 正式推出之前可以还会有调整，

### [#](#_1-createmutablesource) 1 createMutableSource
...
### [#](#_2-usemutablesource) 2 useMutableSource
