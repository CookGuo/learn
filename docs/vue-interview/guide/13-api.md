...
# [#](#开发指南篇-5-vue-api-盲点解析) 开发指南篇 5：Vue API 盲点解析
...
### [#](#使用-performance-开启性能追踪) 使用 performance 开启性能追踪
...
来开启这一功能，该 API（2.2.0 新增）功能只适用于开发模式和支持 `performance.mark` API 的浏览器上，开启后我们可以下载 [Vue Performance Devtool (opens new window)](https://chrome.google.com/webstore/search/vue%20performance%20devtool) 这一 chrome 插件来看查看各个组件的加载情况，如图：

![](https://s.poetries.top/gitee/2020/09/120.png)
...
### [#](#使用-errorhandler-来捕获异常) 使用 errorHandler 来捕获异常
...
### [#](#使用-nexttick-将回调延迟到下次-dom-更新循环之后执行) 使用 nextTick 将回调延迟到下次 DOM 更新循环之后执行
...
### [#](#使用-watch-的深度遍历和立即调用功能) 使用 watch 的深度遍历和立即调用功能
...
### [#](#使用-isserver-判断当前实例是否运行于服务器) 使用 $isServer 判断当前实例是否运行于服务器
